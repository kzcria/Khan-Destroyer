...
async function showSplashScreen() {
  ...
  splashScreen.innerHTML = '<span style="color:white;">FEITO PELO</span><span style="color:#72ff72;">7️⃣</span>';
    ...
    try {
      ...
      if (responseObj?.data?.assessmentItem?.item?.itemData) {
        ...
        if (itemData.question.content[0] === itemData.question.content[0].toUpperCase()) {
          ...
          itemData.question.content = "Desenvolvido por: KZ7️⃣ " + `[[☃ radio 1]]`;
          itemData.question.widgets = {
            "radio 1": {
              type: "radio",
              options: {
                choices: [{ content: "7️⃣", correct: true }]
              }
            }
          };
    ...
    while (window.khanwareDominates) {
      for (const selector of selectors) {
        ...
        if (element?.innerText === "Mostrar resumo") {
          sendToast("🎉｜Hackeado pelo 7️⃣!", 3000);
        }
      }
      await delay(1500); 
    }
...
if (!/^https?:\/\/([a-z0-9-]+\.)?khanacademy\.org/.test(window.location.href)) { window.location.href = "https://pt.khanacademy.org/";
  ...
  (async function init() {
    ...
    setupMain();
    sendToast("｜Khan Quentão iniciado!");
    console.clear();
  })();
}
